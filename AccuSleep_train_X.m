function [net] = AccuSleep_train_X(fileList, SR, epochLen, epochs, imageLocation)
% AccuSleep_train_X  Train a network for classifying brain states
% Zeke Barger, 021321
%
%   Arguments:
...
%   AccuSleep_GUI_X or AccuSleep_classify_X for classification.
...
if ~iscell(fileList) || (size(fileList,1) < 1 || size(fileList,2) ~= 3)
    error(['fileList argument is not correctly formatted.',...
        'Type "help AccuSleep_train_X" for details.'])
end

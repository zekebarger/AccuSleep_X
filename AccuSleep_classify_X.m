function [labels] = AccuSleep_classify_X(EEG, EMG, net, SR, epochLen, calibrationData, minBoutLen)
% AccuSleep_classify_X  Classify brain states
% Zeke Barger 021321
%
%   Arguments: (note that these are data, not paths to files)
%   EEG - a 1-D matrix of EEG data
%   EMG - a 1-D matrix of EMG data
%   net - a trained network. To create one, use AccuSleep_train_X
%   SR - sampling rate (Hz)
...
%       combination of subject and recording equipment. This is the output of
%       createCalibrationData_X.m
...
if minBoutLen > epochLen
    labels = enforceMinDuration_X(labels, ones(1,n_states) * ceil(minBoutLen / epochLen), ...
        1:n_states, 0);
end

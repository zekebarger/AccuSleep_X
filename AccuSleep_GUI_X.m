function varargout = AccuSleep_GUI_X(varargin)
% AccuSleep_GUI_X A GUI for classifying rodent sleep stages
% Zeke Barger, 021321
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @AccuSleep_GUI_X_OpeningFcn, ...
    'gui_OutputFcn',  @AccuSleep_GUI_X_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before AccuSleep_GUI_X is made visible.
function AccuSleep_GUI_X_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for AccuSleep_GUI_X
...
text(handles.axes1,.51,.496,'AccuSleep X','FontSize',42,'Rotation',90,'Color',[.68 .87 .71],...
    'HorizontalAlignment','center')
text(handles.axes1,.49,.504,'AccuSleep X','FontSize',42,'Rotation',90,'Color',[.17 .26 .62],...
    'HorizontalAlignment','center')
...
function varargout = AccuSleep_GUI_X_OutputFcn(hObject, eventdata, handles)
...
message = AccuSleep_viewer_X(eegFile.EEG, emgFile.EMG,...
...
calibrationData = createCalibrationData_X(standardizeSR(eegFile.EEG, oldSR, 128),...
...
for i = 1:length(allRecordings)
    ...
    % run AccuSleep_classify_X on the recording
    newLabels{i} = AccuSleep_classify_X(standardizeSR(eegFile.EEG, oldSR, 128),...
        standardizeSR(emgFile.EMG, oldSR, 128),...
